% Example 10.7.5 - from Van den Hof lecture notes two stage closed loop identification method modified for reference r2 only, and proportional controller

clear;clc;%
% setappdata(0, 'AutoStagger_LRDown_Last', []);   % ensure first figure starts at top-left win_size(0, 'DefaultFigureCreateFcn', @autoStagger_LRDown_relSize);
% close all;

opts1=bodeoptions('cstprefs');opts1.FreqUnits = 'Hz';opts1.XLim={[0.1 50]}; opts1.PhaseMatching='on'; opts1.Grid='on'; % bode plot options
...
for i =[tf(15), 6+60*Ts/(z-1)]%[tf(5)]%  , %tf(0.01),  0.5*(z^-1-0.8*z^-2)] %controller 
    C= i;
    S0 = 1/(1+G0*C)
    ...
    for e = e_vector
        ...
        figure;hold on;
        bodeplot(G0,'r', opts1);
        bodeplot( -1/C, 'r--',opts1);ch = get(gca,'Children');set(ch(1),'DisplayName','-1/C');
        bodeplot(Nonparametric_1 , 'm.', opts1);ch = get(gca,'Children');set(ch(1),'DisplayName',sprintf('Nonparametric (%.0f lags)', win_size_1));
        bodeplot(Nonparametric_2 , 'c.' , opts1);ch = get(gca,'Children');set(ch(1),'DisplayName',sprintf('Nonparametric (%.0f lags)', win_size_2));

        bodeplot( G_direct , 'b-.' , opts1);
        bodeplot(G_2stage , 'g--', opts1);
