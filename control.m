...
G_open_minreal = minreal( G_closed/(Controller*(1-G_closed))) 
G_open_minreal.InputName =  'i_sv';
...
%%
% obs = vpa(obsv(G_open_minreal));
% r_obsv = rank(obs)
% ctrlb = vpa(ctrb(G_open_minreal));
% r_ctrlb = rank(ctrlb)
...
Q = 1*diag([zeros(1,n_states),1]);%blkdiag(eye(nx), eye(ny));
R = 1*eye(size(G_open.B,2));
...
Optimal_closed_loop = connect(plant_aug,  controller , integrator, sumblk1, 'x_ref','y_xT')
...
x_PIDF_15Hz = lsim(G_PIDF_15Hz ,  x_ref , t_vector,'zoh');
...
x_Optimal = lsim(Optimal_closed_loop ,  x_ref , t_vector,'zoh');
