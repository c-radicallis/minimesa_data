...
% bode plot options
opts2=bodeoptions('cstprefs');opts2.PhaseVisible='on';opts2.XLim={[0.05 pi]};opts2.PhaseWrapping="on"; %opts2.PhaseWrappingBranch=-360;
opts2.MagLowerLimMode = 'manual';opts2.MagLowerLim=-20;
...
e_vector = [0.01 , 5 ].*wgn(data_points , 1 , 1 );
...
for i = [tf(5) , 0.08*(z^-1-0.8*z^-2)]%tf(0.01),  0.5*(z^-1-0.8*z^-2)] %controller 
    C= i;
    S0 = 1/(1+G0*C)
