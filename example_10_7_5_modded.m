% Example 10.7.5 - two stage closed loop identification method modified for
% reference r2 only, and proportinal controller
...
for i = [10 , 1  ] %2-stage stops working when S0 is close to instable
    C=tf(i);%*(z^-1 - 0.8*z^-2)
    S0 = 1/(1+G0*C)
    
    % this is to check the effect of feedback intensity vs excitation intensity
    r =  lsim( C , r2/i , t );%reference increases in magnitude when controller gain decreases
    mar = [mar ; mean(abs(r))]
    u_r =  lsim(S0 , r ,t);
