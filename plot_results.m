...
folder_1201_tolmezo ='C:\Users\afons\OneDrive - Universidade de Lisboa\Controlo de Plataforma Sismica\minimesa_data\12-1-2026\tolmezo_scl0.16\';
file = 'tune_cutoff_10Hz.acq'; % load output acq
LTF_to_TXT_then_load_wSV( file , folder_1201_tolmezo );
...
plot(f_vector, picos_ddx_T_tuned_10hz,'--', 'LineWidth' , 2, 'DisplayName',sprintf( 'Tuned PIDF -  MSE= %.2e',      mean((picos_ddx_tgt_T-picos_ddx_T_tuned_10hz).^2 )));% - Normal
plot(f_vector, picos_ddx_T_tuned_15hz,'--', 'LineWidth' , 2, 'DisplayName',sprintf( 'Tuned PIDF -  MSE= %.2e',      mean((picos_ddx_tgt_T-picos_ddx_T_tuned_15hz).^2 )));% - Normal
plot(f_vector, picos_ddx_T_tuned_20hz,'--', 'LineWidth' , 2, 'DisplayName',sprintf( 'Tuned PIDF -  MSE= %.2e',      mean((picos_ddx_tgt_T-picos_ddx_T_tuned_20hz).^2 )));% - Normal
plot(f_vector, picos_ddx_T_acq_4 ,'-', 'LineWidth' , 2, 'Color', color4, 'DisplayName',sprintf( 'Adapted driver 0 - MSE= %.2e', mean((picos_ddx_tgt_T-picos_ddx_T_acq_4).^2 )));
...
plot(f_vector, picos_x_T_tuned_10hz,'--', 'LineWidth' , 2, 'DisplayName',sprintf( 'Tuned PIDF - MSE= %.2e',     mean((picos_x_tgt_T-picos_x_T_tuned_10hz).^2 )));%- Normal
plot(f_vector, picos_x_T_tuned_15hz,'--', 'LineWidth' , 2, 'DisplayName',sprintf( 'Tuned PIDF - MSE= %.2e',     mean((picos_x_tgt_T-picos_x_T_tuned_15hz).^2 )));%- Normal
plot(f_vector, picos_x_T_tuned_20hz,'--', 'LineWidth' , 2, 'DisplayName',sprintf( 'Tuned PIDF - MSE= %.2e',     mean((picos_x_tgt_T-picos_x_T_tuned_20hz).^2 )));%- Normal
