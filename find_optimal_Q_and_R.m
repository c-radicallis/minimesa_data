...
opts1=bodeoptions('cstprefs');opts1.FreqUnits = 'Hz';opts1.XLim={[1 40]};opts1.PhaseWrapping="on";opts1.PhaseWrappingBranch=-360;%opts1.Ylim={[-40 10]};
...
integrator = tf(1,[1 0], Ts); % The integrator integrates the tracking error.
...
for i =  1e4 : -2e3: 1e3
    Q = diag([eye(1,n_states),1e6]);%1e3*diag([zeros(1,n_states),1]);%blkdiag(eye(nx), eye(ny));
    R = i*eye(size(OL_200.B,2));
    ...
    %Optimal_CL = minreal(Optimal_CL);
    bodeplot(Optimal_CL,opts1);

    % Simulation with tuned controllers
    x_sim_T_optimal = lsim(Optimal_CL ,  x_tgt_T, time_vector,'zoh');
    m = max( x_sim_T_optimal);
    if m > max(x_tgt_T)*2
        error('Stopping: Optimal is blowing up! Q = %e \n x_sim= %e', i , m);
    end
    sprintf('x_sim= %e',  m);
end
