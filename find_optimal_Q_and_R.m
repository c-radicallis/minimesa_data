...
opts1=bodeoptions('cstprefs');opts1.FreqUnits = 'Hz';opts1.XLim={[1 100]};opts1.PhaseWrapping="on";opts1.PhaseWrappingBranch=-360;opts.MagUnits = 'dB';
opts.MagLim   = [-40, 10];%opts1.Ylim={[-40 10]};
opts2=bodeoptions('cstprefs');opts2.FreqUnits = 'Hz';opts2.XLim={[0.1 100]}
...
% e_to_sv = iddata( sv2_acq , x_drv_T_0_cut-x_acq_T,Ts); e_to_sv_detrended = detrend(e_to_sv);
% C_est = polyest(e_to_sv_detrended,  [0 3 0 0 0 0],'Ts',Ts);
% sat = idSaturation('LinearInterval',[-16.0920,15.6920]); sat.Free =[0 0];
% C_est_nonLin = nlhw(e_to_sv_detrended, C_est, [], sat);
% C_est_nonLin.LinearModel
...
figure(11);hold on; bodeplot(OL_200,opts2); grid on;
...
%close all; clc;
figure; hold on; 
bodeplot(CL_PIDF_15Hz,opts1); grid on;

for i =  [1, 5, 10] %, 50, 100 ]
% stable=1; i =1e9;
% while stable
%     i= i - 250
    Q = diag([eye(1,n_states) , i]); %eps*eye(1,n_states),Q1 , Q2 ,Q3 , Q4
    R = eye(size(OL_200.B,2)); 
    K_lqi = lqi(OL_200, Q, R)% Design the LQI controller for the original system
    ...
    stable = isstable( Optimal_CL)
    if stable == 0
        error('Stopping: Qi =%e results in unstable closed loop',i);
    else
        bodeplot(Optimal_CL,opts1);
        x_sim_T_optimal = lsim(Optimal_CL ,  x_tgt_T, time_vector,'zoh');
        m = max( x_sim_T_optimal);
        if m > max(x_tgt_T)*2
            error('Stopping: Optimal is blowing up! i = %e \n x_sim= %e', i , m);
        end
    end
end
